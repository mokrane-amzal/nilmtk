...
def convert_smart_meter(smart_meter_path, output_filename, format='HDF'):
    """
    Parameters
    ----------
    smart_meter_path : str
        The root path of the smart_meter dataset.
    output_filename : str
    ...
    def _smart_meter_measurement_mapping_func(house_id, chan_id):
    ...
    _convert(smart_meter_path, store, _smart_meter_measurement_mapping_func, 'Europe/Paris')
    ...
    print("Done converting smart_meter to HDF5!")
...
def _get_csv_filename(input_path, key_obj):
    ...
    Parameters
    ----------
    input_path : (str) the root path of the smart_meter dataset
...
def _load_csv(filename, columns, tz, drop_duplicates=False, sort_index=False):
    ...
    if sort_index:
        df = df.sort_index() # raw smart_meter data isn't always sorted
